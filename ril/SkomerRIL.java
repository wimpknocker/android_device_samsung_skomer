...
public class SkomerRIL extends RIL implements CommandsInterface {
    ...
    protected HandlerThread mSkomerRILThread;
    protected ConnectivityHandler mSkomerRILHandler;
    
    private Message mPendingGetSimStatus;

    public SkomerRIL(Context context, int networkMode, int cdmaSubscription) {
        super(context, networkMode, cdmaSubscription);
        mQANElements = 5;
    }

    public SkomerRIL(Context context, int preferredNetworkType, int cdmaSubscription, Integer instanceId) {
    ...
    public void setPreferredNetworkType(int networkType , Message response) {
        ...
        if(NeedReconnect())
        {
            if (mSkomerRILHandler == null) {

                handlerThread = new HandlerThread("mSkomerRILThread");
                mSkomerRILThread = handlerThread;

                mSkomerRILThread.start();

                looper = mSkomerRILThread.getLooper();
                mSkomerRILHandler = new ConnectivityHandler(mContext, looper);
            }
            mSkomerRILHandler.setPreferedNetworkType(networkType, response);
        } else {
            if (mSkomerRILHandler != null) {
                mSkomerRILThread = null;
                mSkomerRILHandler = null;
            }
